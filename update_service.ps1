...
function Update-DataJs {
    ...
    for ($i=0; $i -lt $maxRetries; $i++) {
        try {
            ...
            Write-Host "data.js updated at $(Get-Date)"
            break
        } catch {
...
Update-DataJs
Write-Host "高级监控服务已启动..."
while ($true) { Start-Sleep -Seconds 1 }
'@

Set-Content -Path "E:\YS-刘伟\WEB\share\watch_service.ps1" -Value $scriptContent -Encoding UTF8
Write-Host "全新的高级监控服务 watch_service.ps1 已生成"
